import {DownOutlined, LockOutlined, LogoutOutlined, SettingOutlined, UserOutlined} from '@ant-design/icons';
import {history, useModel} from '@umijs/max';
import {Avatar, Menu, Spin} from 'antd';
import type {ItemType} from 'antd/es/menu/hooks/useItems';
import type {MenuInfo} from 'rc-menu/lib/interface';
import React, {useCallback} from 'react';
import {flushSync} from 'react-dom';
import HeaderDropdown from '../HeaderDropdown';
import styles from './index.less';
import {userLogoutUsingPOST} from "@/services/yuapi-backend/userController";

export type GlobalHeaderRightProps = {
  menu?: boolean;
};

const AvatarDropdown: React.FC<GlobalHeaderRightProps> = ({menu}) => {
  /**
   * 退出登录，并且将当前的 url 保存
   */
  const loginOut = async () => {
    await userLogoutUsingPOST();
    /** 此方法会跳转到 redirect 参数所在的位置 */
    window.location.href="/User/Login"    // Note: There may be security issues, please note
  };
  const {initialState, setInitialState} = useModel('@@initialState');

  const handleDownload = () => {
    const fileUrl = 'sdk/api-client-sdk-0.0.1.jar';
    const link = document.createElement('a');
    link.href = fileUrl;
    link.download = "api-client-sdk-0.0.1.jar";
    link.target = '_blank';
    link.rel = 'noopener noreferrer';
    link.click();
  };

  const onMenuClick = useCallback(
    (event: MenuInfo) => {
      const {key} = event;
      // console.log("key :: " + key)
      if (key === 'logout') {
        flushSync(() => {
          setInitialState((s) => ({...s, currentUser: undefined}));
        });
        loginOut();
        history.push('/User/Login');
        return;
      }
      if (key === 'download') {
        handleDownload();
        return;
      }
      // history.push(`/account/${key}`);
    },
    [setInitialState],
  )

  const loading = (
    <span className={`${styles.action} ${styles.account}`}>
      <Spin
        size="small"
        style={{
          marginLeft: 8,
          marginRight: 8,
        }}
      />
    </span>
  );

  if (!initialState) {
    return loading;
  }
  const { loginUser } = initialState;

  // if (!loginUser || !loginUser.userName) {
  if (!loginUser || !loginUser.userAccount) {
    return loading;
  }
  // const ak =

  const menuItems: ItemType[] = [
    ...( [
        // {
        //   key: 'download',
        //   icon: <SettingOutlined/>,
        //   label: 'SDK下载',
        // },
        {
          key: 'ak',
          icon: <LockOutlined/>,
          label: 'accessKey',
        },
        {
          key: 'ak',
          // icon: <UserOutlined/>,
          label: loginUser.accessKey,
        },
        {
          key: 'sk',
          icon: <LockOutlined/>,
          label: 'secretKey',
        },
        {
          key: 'sk',
          // icon: <UserOutlined/>,
          label: loginUser.secretKey,
        },
        {
          type: 'divider' as const,
        },
        {
          key: 'logout',
          icon: <LogoutOutlined/>,
          label: '退出登录',
        },
      ]
    ),
  ];

  const menuHeaderDropdown = (
    <Menu className={styles.menu} selectedKeys={[]} onClick={onMenuClick} items={menuItems}/>
  );

  return (
    <HeaderDropdown overlay={menuHeaderDropdown}>
      <span className={`${styles.action} ${styles.account}`}>
        <Avatar size="small" className={styles.avatar} src={loginUser.userAvatar} alt="avatar" />
        <span className={`${styles.name} anticon`}>{loginUser.userAccount}</span>
      </span>
    </HeaderDropdown>
  );
};

export default AvatarDropdown;
